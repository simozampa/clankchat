...
describe('harness adapters', () => {
  ...
  it('does not install OpenCode filesystem or tool guards', async () => {
    const module = (await import(
      `data:text/javascript;base64,${Buffer.from(OPENCODE_PLAN_PLUGIN).toString('base64')}`
    )) as {
      SameTreePlanPublisher: (input: Record<string, unknown>) => Promise<{
    ...
    await plugin['tool.execute.before'](
      ...
      { args: { command: 'ln -sf ~/.opencode/bin/opencode ~/.local/bin/opencode' } },
    );

    expect(plugin).not.toHaveProperty('shell.env');
