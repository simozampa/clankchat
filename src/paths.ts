...
export function assertSafeWritePath(repositoryRoot: string, target: string): string {
  ...
  if (!isInside(root, absolute)) {
    throw new ClankChatError('INVALID_INPUT', 'A generated path cannot leave the repository.', {
      path: target,
    });
  }
  ...
  for (const segment of segments) {
    ...
    try {
      if (lstatSync(current).isSymbolicLink()) {
        throw new ClankChatError('INVALID_INPUT', 'Refusing to write through a symbolic link.', {
          path: current,
        });
      }
    } catch (error) {
